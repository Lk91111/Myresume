...
class CI_Pagination{
    ...
    function createLinks(){ 
        ...
        if($this->currentPage > $this->numLinks){
            ...
            $output .= $this->firstTagOpen.'<a href="'.$firstPageURL.'" class="links">'.$this->firstLink.'</a>'.$this->firstTagClose;
        }
        // Render the "previous" link
        if($this->currentPage != 1){
            $i = ($uriPageNum - 1);
            if($i == 0) $i = '';
            $output .= $this->prevTagOpen.'<a href="'.$this->baseURL.$i.'" class="links">'.$this->prevLink.'</a>'.$this->prevTagClose;
        }
        ...
        for($loop = $start -1; $loop <= $end; $loop++){
            ...
            if($i >= 1){
                if($this->currentPage == $loop){
                    ...
                    $output .= $this->numTagOpen.'<a href="'.$this->baseURL.$i.'" class="links">'.$loop.'</a>'.$this->numTagClose;
                }
            }
        }
        ...
        if($this->currentPage < $numPages){
            ...
            $output .= $this->nextTagOpen.'<a href="'.$this->baseURL.$i.'" class="links">'.$this->nextLink.'</a>'.$this->nextTagClose;
        }
        // Render the "Last" link
        if(($this->currentPage + $this->numLinks) < $numPages){
            $i = $numPages;
            $output .= $this->lastTagOpen.'<a href="'.$this->baseURL.$i.'" class="links">'.$this->lastLink.'</a>'.$this->lastTagClose;
        }
